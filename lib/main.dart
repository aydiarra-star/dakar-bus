...
class AppColors {
  ...
  static const terLine = Color(0xFF8B4513);      // Marron / Orange-Gris
  static const brtLine = Color(0xFF2E7D32);      // Vert
  static const dddLine = Color(0xFF1565C0);      // Bleu
  static const tataLine = Color(0xFF757575);     // Blanc / Gris
...
final List<Stop> demoStops = [
  ...
  const Stop(
    id: '5',
    name: 'Arrêt Tata Ligne 23',
    category: 'AFTU',
    lines: 'Tata 23',
    distance: '280 m',
    schedule: '08:42 (dans 6 min)',
...
class _MainShellState extends State<MainShell> {
  int _currentIndex = 0;
  final List<String> _alerts = ['1', '2', '3'];
      ...
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            const Text('Prochains départs en temps réel',
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(stop.direction, style: const TextStyle(fontSize: 12, color: AppColors.textSecondary)),
                  ],
                ),
              ),
              Column(
              ...
              child: ElevatedButton.icon(
                onPressed: () {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('🤖 Assistant IA : Prochain TER vers Diamniadio dans 4 min !')),
                  );
                },
            ...
            if (alertStops.isEmpty)
              ...
              ...alertStops.map((s) => Card(
                    ...
                    child: ListTile(
                      ...
                      title: Text(s.name),
                      subtitle: const Text('Prévenu 10 min avant le passage'),
                      trailing: IconButton(
...
class TripsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const Scaffold(
      body: Center(child: Text('Recherche de trajets disponible.')),
    );
  }
}
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(stop.direction, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            Text('Prochain passage : ${stop.schedule}', style: const TextStyle(fontSize: 16, color: AppColors.success)),
            Text('Tarif : ${stop.price}', style: const TextStyle(fontSize: 16)),
          ],
        ),
      ),
    );
