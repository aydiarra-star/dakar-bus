...
function scheduledSentence({ network, lineNumber, stopName, timeHHMM, terminus, source, sourceTypeLabel, validity, status = STATUS.SCHEDULED }) {
  let s = `La ligne ${network} ${lineNumber} dessert cet itinéraire. Depuis l'arrêt ${stopName}, le prochain départ ${status === 'REAL_TIME' ? 'annoncé en temps réel' : 'programmé'} est à ${timeHHMM}.`;
...
function answerScheduleQuestion(provider, questionOrText, options = {}) {
  ...
  for (const { source, routes } of bySource.values()) {
    ...
    for (const route of routes) {
      for (const stop of stops) {
        ...
        // REAL_TIME is only relayed from a current provider result, never inferred.
        if (res.status === STATUS.SCHEDULED || (res.status === 'REAL_TIME' && res.isCurrent)) {
        ...
        } else if (res.status === STATUS.ESTIMATED && res.estimate) {
          bestEstimate ||= res;
    ...
    if (filtered.length) {
      ...
      const sentence = scheduledSentence({
        ...
        terminus: d.direction.terminusStopName, source: d.source, sourceTypeLabel: d.sourceType, validity: d.validity, status: best.result.status,
      });
      return reply(best.result.status, null, sentence, {
        network,
  ...
  // Do not invent a stop time from a frequency. Exact departures and
  // stop ambiguity retain priority over this line-level estimate.
  if (bestEstimate) {
    return reply(STATUS.ESTIMATED, null, estimatedSentence({ network: bestEstimate.estimate.network, lineNumber: question.lineNumber, estimate: bestEstimate.estimate }), {
      network: bestEstimate.estimate.network, provenanceLevel: bestEstimate.provenanceLevel,
      source: bestEstimate.source, estimate: bestEstimate.estimate, reference: ref,
    });
  }
