// Simulation v6 : test complet du service worker et des tracés v6
//
...
//     A.aS2, puis résolues via le fetch handler (TER 13 gares, B1 23 stations,
//     DDD/TATA = JSON) ;
...
//   * longueurs TER (~35 km) et BRT (~17.5 km, corridor 23 stations).
//
...
ok(`${network.stops.length}/${network.stops.length} arrets valides par eI`, invalidStops.length === 0, invalidStops.map((s) => s.id).join(','));
...
ok('demo B1 = JSON B1 direct (23 stations réelles)',
...
ok('formes en mode secours-consecutif',
...
// Longueurs : TER ~34.6 km (plage 30-40) ; BRT 23 stations ~17.5 km (plage 16-20).
...
ok(`BRT site propre 23 stations : ${(brtLen / 1000).toFixed(1)} km (plage 16-20)`, brtLen >= 16000 && brtLen <= 20000);
...
ok('TRACES_SW=v6', api.TRACES_SW === 'v6');
ok('cache=osrm-geom-v6 (renouvellement force)', api.OSRM_CACHE === 'osrm-geom-v6');
...
for (const route of network.routes) {
  ...
  if (route.operator_id === 'ter' || route.operator_id === 'brt') {
    // v6 : secours = EXACTEMENT les gares/arrets consecutifs, dans l'ordre.
...
// Démo B1 : 23 stations = 22 segments
const b1DemoPts = [];
for (const d of demoPairs.filter((x) => x.demo === 'B1')) {
  b1DemoPts.push(...(geomCache.get(d.coords) ?? []));
}
ok(`demo B1 : ${b1DemoPts.length} points = 22 segments x 2 stations consecutives`, b1DemoPts.length === 44);
...
// --- 12. SW dégradé + chemin legacy cross-origin -----------------
