// Simulation v4 : reprend la simulation v3 (VRAI flutter_service_worker.js dans
// un VM Node) et ajoute les contrôles spécifiques du correctif v4 :
//
...
//     DDD/TATA = JSON) ;
//   * séparation par mode : paires TER/BRT = formes locales (shape_source +
//     provenance), paires bus = osrm-driving ;
...
const trackBad = [...trackUrls].filter((u) => !(prov[u] ?? '').endsWith('-v4'));
const roadBad = pairs.filter((u) => !trackUrls.has(u) && prov[u] !== 'osrm-driving');
ok(`${trackUrls.size} paires TER/BRT = formes locales`, trackBad.length === 0, trackBad.slice(0, 2).join(' | '));
...
ok('TRACES_SW=v4', api.TRACES_SW === 'v4');
ok('cache=osrm-geom-v4', api.OSRM_CACHE === 'osrm-geom-v4');
...
ok(`${snappedLines}/${network.routes.length} lignes tracees sur geometrie reelle`,
...
ok(`demo TER : ${terDemoPts.length} points ferroviaires (pas de corde droite)`, terDemoPts.length > 20);
